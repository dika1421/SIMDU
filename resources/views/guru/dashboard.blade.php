...
<div class="row g-4 mb-4">
    <div class="col-xl-3 col-md-6">
        <div class="stat-card card h-100" onclick="window.location.href='{{ route('guru.nilai.index') }}'">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div class="bg-primary bg-opacity-10 rounded p-3">
                        <i class="fas fa-chalkboard fa-2x text-primary"></i>
                    </div>
                    <h2 class="mb-0 fw-bold">{{ $totalKelas ?? 0 }}</h2>
                </div>
    ...
    <div class="col-xl-3 col-md-6">
        <div class="stat-card card h-100" onclick="window.location.href='{{ route('guru.absensi.index') }}'">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div class="bg-success bg-opacity-10 rounded p-3">
                        <i class="fas fa-users fa-2x text-success"></i>
                    </div>
                    <h2 class="mb-0 fw-bold">{{ $totalSiswa ?? 0 }}</h2>
                </div>
                <h6 class="text-muted mb-1">Total Siswa</h6>
                <small class="text-info">
                    <i class="fas fa-users me-1"></i> {{ $siswaPerKelas ?? 0 }} per kelas rata-rata
                </small>
            </div>
        </div>
    </div>
        ...
        <div class="stat-card card h-100" onclick="window.location.href='{{ route('guru.nilai.index') }}'">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <h2 class="mb-0 fw-bold">{{ $totalMapel ?? 0 }}</h2>
                </div>
        ...
        <div class="stat-card card h-100">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <h2 class="mb-0 fw-bold">{{ number_format($rataNilai ?? 0, 1) }}</h2>
                </div>
                ...
                <div class="progress progress-sm mt-2">
                    ...
                    @endphp
                    <div class="progress-bar bg-info" style="width: {{ $persentase }}%"></div>
                </div>
                <small class="text-{{ ($rataNilai ?? 0) >= 75 ? 'success' : 'warning' }} mt-1 d-block">
                    @if(($rataNilai ?? 0) >= 75)
                ...
                @if(isset($jadwalHariIni) && count($jadwalHariIni) > 0)
                    <div class="list-group list-group-flush">
                        @foreach($jadwalHariIni as $j)
                            <div class="list-group-item schedule-item px-0" style="border-left-color: {{ $loop->index % 2 == 0 ? '#667eea' : '#764ba2' }}; border-left-width: 3px;">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <p class="mb-0 fw-bold">{{ $j->mapel->nama_mapel ?? '-' }}</p>
                                        <small class="text-muted">
                                            <i class="fas fa-users me-1"></i> {{ $j->kelas->nama_kelas ?? '-' }}
                                            <i class="fas fa-door-open ms-2 me-1"></i> {{ $j->ruangan ?? 'Ruang 1' }}
                                        </small>
                                    </div>
                                    <div>
                                        <a href="{{ route('guru.absensi.index') }}" class="btn btn-sm btn-outline-primary">
            ...
            <div class="card-body">
                <div class="row g-3 mb-4">
                    <div class="col-6">
                        <div class="attendance-stat text-center p-3 bg-success bg-opacity-10 rounded">
                            <h3 class="mb-0 text-success fw-bold">{{ $hadirHariIni ?? 0 }}</h3>
                            <small class="text-muted">
                    ...
                    <div class="col-6">
                        <div class="attendance-stat text-center p-3 bg-warning bg-opacity-10 rounded">
                            <h3 class="mb-0 text-warning fw-bold">{{ $sakitHariIni ?? 0 }}</h3>
                            <small class="text-muted">
                    ...
                    <div class="col-6">
                        <div class="attendance-stat text-center p-3 bg-info bg-opacity-10 rounded">
                            <h3 class="mb-0 text-info fw-bold">{{ $izinHariIni ?? 0 }}</h3>
                            <small class="text-muted">
                    ...
                    <div class="col-6">
                        <div class="attendance-stat text-center p-3 bg-danger bg-opacity-10 rounded">
                            <h3 class="mb-0 text-danger fw-bold">{{ $alfaHariIni ?? 0 }}</h3>
                            <small class="text-muted">
                ...
                <div class="mt-3">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <small class="fw-bold">{{ $persentaseKehadiran ?? 0 }}%</small>
                    </div>
                ...
                <div class="mt-3 text-center">
                    ...
                    <a href="{{ route('guru.absensi.riwayat') }}" class="btn btn-sm btn-outline-secondary ms-2">
        ...
        <div class="card h-100">
            ...
            <div class="card-body">
                <div class="list-group list-group-flush">
                    <div class="list-group-item px-0">
                        <div class="d-flex justify-content-between">
                            ...
                            <span class="fw-bold">{{ $nilaiTerinput ?? 0 }} / {{ $totalNilai ?? 0 }}</span>
                        </div>
                        <div class="progress progress-sm mt-2">
                            @php
                                $progressNilai = ($nilaiTerinput ?? 0) / max(($totalNilai ?? 1), 1) * 100;
                            @endphp
                            <div class="progress-bar bg-success" style="width: {{ $progressNilai }}%"></div>
                        </div>
                    </div>
                    <div class="list-group-item px-0">
                        <div class="d-flex justify-content-between">
                            <span><i class="fas fa-calendar-check text-warning me-2"></i>Hari Efektif</span>
                            <span class="fw-bold">{{ $hariEfektif ?? 0 }} / {{ $totalHari ?? 30 }}</span>
                        </div>
                        <div class="progress progress-sm mt-2">
                            @php
                                $progressHari = ($hariEfektif ?? 0) / max(($totalHari ?? 1), 1) * 100;
                            @endphp
                            <div class="progress-bar bg-warning" style="width: {{ $progressHari }}%"></div>
                        </div>
                    </div>
                </div>
                    ...
                    @foreach($prestasiTerbaru as $p)
                        <div class="d-flex mb-2">
                            ...
                            <div class="flex-grow-1 ms-2">
                                <small class="fw-bold">{{ $p->nama_siswa }}</small>
                                <small class="text-muted d-block">{{ $p->nama_prestasi }}</small>
                            </div>
                        </div>
                    @endforeach
                @else
                ...
                <div class="d-grid gap-2">
                    <a href="{{ route('guru.nilai.input') }}" class="btn btn-primary btn-sm">
                        <i class="fas fa-edit"></i> Input Nilai
                    </a>
                    <a href="{{ route('guru.komunikasi.create') }}" class="btn btn-outline-info btn-sm">
