...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-2 pb-3">
    ...
    <div>
        <a href="{{ route('guru.komunikasi.create') }}" class="btn btn-primary btn-action">
            <i class="fas fa-plus me-1"></i> Pesan Baru
        </a>
        @if(isset($belumDibaca) && $belumDibaca > 0)
            <a href="{{ route('guru.komunikasi.mark-all-read') }}" class="btn btn-outline-secondary btn-action">
...
<div class="row g-3 mb-4">
    <div class="col-md-3">
        <div class="stat-card primary">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <div class="stat-number">{{ ($pesanMasuk->total() ?? 0) + ($pesanKeluar->total() ?? 0) }}</div>
    ...
    <div class="col-md-3">
        <div class="stat-card info">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <div class="stat-number">{{ $pesanMasuk->total() ?? 0 }}</div>
    ...
    <div class="col-md-3">
        <div class="stat-card success">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <div class="stat-number">{{ $pesanKeluar->total() ?? 0 }}</div>
    ...
    <div class="col-md-3">
        <div class="stat-card warning">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <div class="stat-number">{{ $belumDibaca ?? 0 }}</div>
...
<div class="tab-content" id="messageTabsContent">
    ...
    <div class="tab-pane fade show active" id="inbox" role="tabpanel">
        ...
        @else
            ...
            @endphp
            <div class="card message-card mb-3 {{ !$sudahDibaca ? 'unread' : '' }}">
                <div class="card-body py-3">
                    <div class="row align-items-center">
                        <div class="col-md-1">
                            <div class="avatar bg-{{ ['primary', 'success', 'warning', 'danger', 'purple'][$p->id % 5] }}">
                                {{ strtoupper(substr($p->pengirim->name ?? 'U', 0, 1)) }}
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="fw-semibold">{{ $p->pengirim->name ?? 'Pengirim' }}</div>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="message-preview">{{ Str::limit($p->isi, 80) }}</div>
                        </div>
                        <div class="col-md-2 text-end">
                            <span class="badge bg-light text-dark">
                                {{ $isPengirim ? 'Dikirim' : 'Diterima' }}
                            </span>
                        </div>
                        <div class="col-md-2 text-end">
                            <a href="{{ route('guru.komunikasi.show', $p->id) }}" 
                ...
                <div class="modal-dialog modal-sm">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="text-center py-3">
                                ...
                                <p class="text-muted small">
                                    <strong>Judul:</strong> {{ $p->judul }}<br>
                                    <strong>Dari:</strong> {{ $p->pengirim->name ?? 'Pengirim' }}
                                </p>
                        ...
                        <div class="modal-footer">
                            ...
                            <form action="{{ route('guru.komunikasi.destroy', $p->id) }}" method="POST" style="display: inline;">
                                @csrf
                                @method('DELETE')
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-3">
                            <div class="fw-semibold">
                                ...
                                @endphp
                                {{ $penerimaFirst ? $penerimaFirst->penerima->name : 'Penerima' }}
                            </div>
                            <div class="message-meta">
                                <i class="far fa-clock me-1"></i>
                                {{ \Carbon\Carbon::parse($p->created_at)->diffForHumans() }}
                            ...
                            <small class="text-muted">
                                {{ $p->jenis == 'broadcast' ? 'Broadcast' : 'Personal' }}
                            </small>
                        </div>
                        <div class="col-md-4">
                            <div class="message-subject">{{ $p->judul }}</div>
                            <div class="message-preview">{{ Str::limit($p->isi, 80) }}</div>
                        </div>
                        <div class="col-md-2 text-end">
                            <span class="badge bg-light text-dark">
                                {{ $p->penerimaPesan->count() }} penerima
                            </span>
                        </div>
                        <div class="col-md-2 text-end">
                            <a href="{{ route('guru.komunikasi.show', $p->id) }}" 
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="text-center py-3">
                                ...
                                <p class="text-muted small">
                                    <strong>Judul:</strong> {{ $p->judul }}<br>
                                    <strong>Kepada:</strong> {{ $p->penerimaPesan->count() }} penerima
                                </p>
                                <p class="text-danger small mb-0">
                                    <i class="fas fa-info-circle me-1"></i>
                                    Pesan yang dihapus tidak dapat dikembalikan!
...
<script>
    $(document).ready(function() {
        ...
        setInterval(function() {
            $.ajax({
                url: '{{ route("guru.komunikasi.unread-count") }}',
                method: 'GET',
