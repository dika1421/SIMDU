...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('administrasi.absensi-sholat.rekap-siswa') }}" class="btn btn-info">
            <i class="fas fa-chart-line me-1"></i> Rekap Siswa
        </a>
        <a href="{{ route('administrasi.absensi-sholat.rekap-guru') }}" class="btn btn-info">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="role" class="form-select">
                            <option value="siswa" {{ request('role', 'siswa') == 'siswa' ? 'selected' : '' }}>
...
<div class="row mb-4">
    <div class="col-md-3 col-sm-6">
        <div class="stat-card-sholat animate-fade-in" style="animation-delay: 0.1s">
            ...
            <div class="stat-number-sholat">{{ number_format($statistik['totalUsers'] ?? 0) }}</div>
            <small>Total {{ request('role', 'siswa') == 'siswa' ? 'Siswa' : 'Guru' }}</small>
            <div class="mt-2">
    ...
    <div class="col-md-3 col-sm-6">
        <div class="stat-card-sholat animate-fade-in" style="background: linear-gradient(135deg, #28a745, #20c997); animation-delay: 0.2s">
            ...
            <div class="stat-number-sholat">{{ number_format($statistik['tepatWaktu'] ?? 0) }}</div>
            <small>Tepat Waktu</small>
            <div class="mt-2">
                <div class="progress" style="height: 4px;">
                    <div class="progress-bar bg-white" style="width: {{ $statistik['totalUsers'] > 0 ? ($statistik['tepatWaktu'] / $statistik['totalUsers']) * 100 : 0 }}%"></div>
                </div>
            </div>
        </div>
    </div>
    <div class="col-md-3 col-sm-6">
        <div class="stat-card-sholat animate-fade-in" style="background: linear-gradient(135deg, #ffc107, #fd7e14); animation-delay: 0.3s">
            ...
            <div class="stat-number-sholat">{{ number_format($statistik['terlambat'] ?? 0) }}</div>
            <small>Terlambat</small>
            <div class="mt-2">
                <div class="progress" style="height: 4px;">
                    <div class="progress-bar bg-white" style="width: {{ $statistik['totalUsers'] > 0 ? ($statistik['terlambat'] / $statistik['totalUsers']) * 100 : 0 }}%"></div>
                </div>
            </div>
        </div>
    </div>
    <div class="col-md-3 col-sm-6">
        <div class="stat-card-sholat animate-fade-in" style="background: linear-gradient(135deg, #dc3545, #c82333); animation-delay: 0.4s">
            ...
            <div class="stat-number-sholat">{{ number_format($statistik['tidakHadir'] ?? 0) }}</div>
            <small>Tidak Hadir</small>
            <div class="mt-2">
                <div class="progress" style="height: 4px;">
                    <div class="progress-bar bg-white" style="width: {{ $statistik['totalUsers'] > 0 ? ($statistik['tidakHadir'] / $statistik['totalUsers']) * 100 : 0 }}%"></div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="row text-center">
            <div class="col-md-2 col-6 mb-3">
                <div class="p-3 rounded-3 bg-light">
                    ...
                    <h4 class="text-primary mb-0">{{ $jadwal->subuh ?? '04:30' }}</h4>
                </div>
            </div>
            <div class="col-md-2 col-6 mb-3">
                <div class="p-3 rounded-3 bg-light">
                    ...
                    <h4 class="text-warning mb-0">{{ $jadwal->dzuhur ?? '12:00' }}</h4>
                </div>
            </div>
            <div class="col-md-2 col-6 mb-3">
                <div class="p-3 rounded-3 bg-light">
                    ...
                    <h4 class="text-orange mb-0">{{ $jadwal->ashar ?? '15:30' }}</h4>
                </div>
            </div>
            <div class="col-md-2 col-6 mb-3">
                <div class="p-3 rounded-3 bg-light">
                    ...
                    <h4 class="text-danger mb-0">{{ $jadwal->maghrib ?? '18:00' }}</h4>
                </div>
            </div>
            <div class="col-md-2 col-6 mb-3">
                <div class="p-3 rounded-3 bg-light">
                    ...
                    <h4 class="text-dark mb-0">{{ $jadwal->isya ?? '19:30' }}</h4>
                </div>
            </div>
            <div class="col-md-2 col-6 mb-3">
                <div class="p-3 rounded-3 bg-primary text-white">
                    ...
                    <h4 class="mb-0">{{ $statistik['totalUsers'] > 0 ? round((($statistik['tepatWaktu'] + $statistik['terlambat']) / $statistik['totalUsers']) * 100, 1) : 0 }}%</h4>
                </div>
            </div>
        </div>
    </div>
...
<div class="row">
    ...
    <div class="col-md-6 col-lg-4">
        <div class="sholat-card sholat-{{ $sholat }} animate-fade-in" style="animation-delay: {{ 0.6 + ($index * 0.1) }}s">
            <div class="sholat-header">
                ...
                <div>
                    <span class="sholat-time">
                        <i class="fas fa-clock me-1"></i>
                        {{ $jadwal->$sholat ?? ($sholat == 'subuh' ? '04:30' : ($sholat == 'dzuhur' ? '12:00' : ($sholat == 'ashar' ? '15:30' : ($sholat == 'maghrib' ? '18:00' : '19:30')))) }}
                    </span>
                    <span class="badge bg-light text-dark ms-2">
                        {{ isset($absensi[$sholat]) ? $absensi[$sholat]->count() : 0 }}/{{ $statistik['totalUsers'] ?? 0 }}
                    </span>
                </div>
            </div>
            <div class="list-group list-group-flush" style="max-height: 300px; overflow-y: auto;">
                ...
                <div class="list-group-item">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <strong>
                                {{ $role == 'siswa' ? ($absen->user->user->name ?? $absen->user->nama ?? '-') : ($absen->user->user->name ?? $absen->user->nama_lengkap ?? '-') }}
                            </strong>
                            <br>
                        ...
                        <span class="status-{{ $absen->status == 'tepat_waktu' ? 'tepat' : ($absen->status == 'terlambat' ? 'terlambat' : 'tidak') }}">
                            <i class="fas {{ $absen->status == 'tepat_waktu' ? 'fa-check-circle' : ($absen->status == 'terlambat' ? 'fa-clock' : 'fa-times-circle') }} me-1"></i>
                            {{ $absen->status == 'tepat_waktu' ? 'Tepat Waktu' : ($absen->status == 'terlambat' ? 'Terlambat' : 'Tidak Hadir') }}
                        </span>
                    </div>
                </div>
                @empty
