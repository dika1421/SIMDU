            ...
            <div class="col-md-4">
                ...
                <select name="bulan" class="form-select">
                    ...
                    @foreach($bulanList as $key => $nama)
                        <option value="{{ $key }}" {{ ($bulan ?? date('m')) == $key ? 'selected' : '' }}>
                            {{ $nama }}
                        </option>
                    @endforeach
                </select>
...
<div class="card">
    <div class="card-header bg-white">
        <h5 class="mb-0">
            ...
            @endphp
            Bulan {{ $namaBulan[$bulan ?? date('m')] }} {{ $tahun ?? date('Y') }}
        </h5>
    </div>
    <div class="card-body">
            ...
            <table class="table table-bordered datatable" id="sppTable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $d->siswa->kelas->nama ?? $d->siswa->kelas->nama_kelas ?? '-' }}</td>
                        ...
                        <td>
                            <span class="badge bg-{{ ($d->status ?? 'belum_lunas') == 'lunas' ? 'success' : 'danger' }}">
                                {{ ($d->status ?? 'belum_lunas') == 'lunas' ? 'Lunas' : 'Belum Lunas' }}
                            </span>
                        </td>
                    </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-4">
                                ...
                                <p class="text-muted mb-0">Belum ada data pembayaran SPP untuk periode ini</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                ...
                <tfoot>
                    <tr>
                        ...
                        <th colspan="1">Rp {{ number_format($total ?? 0, 0, ',', '.') }}</th>
                        <th colspan="3"></th>
                    </td>
                </tfoot>
                @endif
