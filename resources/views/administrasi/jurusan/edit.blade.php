...
<div class="card">
    <div class="card-body">
        <form action="{{ route('administrasi.jurusan.update', $jurusan->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="kode_jurusan" class="form-control @error('kode_jurusan') is-invalid @enderror" 
                           value="{{ old('kode_jurusan', $jurusan->kode_jurusan) }}" placeholder="Contoh: PMN, TBG, JBG">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror" 
                           value="{{ old('nama', $jurusan->nama) }}" placeholder="Contoh: Pemasaran, Tata Boga, Jasa Boga" required>
                    @error('nama')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                        ...
                        <option value="nonaktif" {{ old('status', $jurusan->status) == 'nonaktif' ? 'selected' : '' }}>Non Aktif</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="kepala_jurusan_id" class="form-select @error('kepala_jurusan_id') is-invalid @enderror">
                        ...
                        @foreach($guruList as $guru)
                            <option value="{{ $guru->id }}" {{ old('kepala_jurusan_id', $jurusan->kepala_jurusan_id) == $guru->id ? 'selected' : '' }}>
                ...
                <div class="col-12 mb-3">
                    ...
                    <textarea name="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror" rows="3" 
                              placeholder="Informasi tambahan tentang jurusan">{{ old('deskripsi', $jurusan->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
